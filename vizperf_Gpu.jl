...
getname(d, n, np; pth="timings_gpu") = joinpath(pth, (d[n])[np])
...
let 
    for nt in  (4,)
        ...
        d = Dict{Int, Dict{Int, String}}(
            n => Dict{Int, String}(
                np => "CUDA_timings_n$(n)_np$(np)" for np in np
            ) 
            for n in n
        )
        ...
        fig
        save("timings_gpu_advection_new.png", fig)
        ...
        fig
        save("timings_gpu_g2p_new.png", fig)
        ...
        fig
        save("timings_gpu_p2g_new.png", fig)
    end
end
