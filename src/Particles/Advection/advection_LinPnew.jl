...
function advection_LinP!(
    ...
    Vxc = zeros(ni.+2...)
    Vyc = zeros(ni.+2...)
    @views @. Vxc[2:end-1, :] = (Vx[1:end-1,:] + Vx[2:end,:]) / 2
    @views @. Vxc[1, :] = Vx[2,:]
    @views @. Vxc[end, :] = Vx[end-1,:]

    @views @. Vyc[:, 2:end-1] = (Vy[:, 1:end-1] + Vy[:, 2:end]) / 2
    @views @. Vyc[:, 1]   = Vy[:, 2]
    @views @. Vyc[:, end] = Vy[:, end-1]
    Vc = (Vxc, Vyc)

    # launch parallel advection kernel
    @parallel (@idx ni) advection_kernel_LinP!(
        coords, method, V, Vc, index, grid_vi, local_limits, dxi, dt
    )
...
@parallel_indices (I...) function advection_kernel_LinP!(
    ...
    for ipart in cellaxes(index)
        ...
        # # advect particle
        pᵢ_new = advect_particle_LinP(
            method, pᵢ, V, Vc, grid, local_limits, dxi, dt, I
        )
...
@inline function advect_particle_LinP(
        method::RungeKutta2,
        p0::NTuple{N},
        V::NTuple{N},
        Vc::NTuple{N},
    ...
    vp0 = interp_velocity2particle_LinP(p0, grid_vi, local_limits, dxi, V, Vc, idx)
...
@inline function interp_velocity2particle_LinP(
        particle_coords::NTuple{N}, grid_vi, local_limits, dxi, V::NTuple{N}, Vc::NTuple{N}, idx::NTuple{N}
    ) where {N}
    return ntuple(Val(N)) do i
        Base.@_inline_meta
        local_lims = local_limits[i]
        v = if check_local_limits(local_lims, particle_coords)
            interp_velocity2particle_LinP(particle_coords, grid_vi[i], dxi, V[i], Vc[i], Val(i), idx)
        else
            Inf
        end
    end
...
@inline function interp_velocity2particle_LinP(
        p_i::Union{SVector, NTuple}, xi_vx::NTuple, dxi::NTuple, F::AbstractArray, Fc::AbstractArray, ::Val{N}, idx
    ) where {N}

    #### Velocity from velocity nodes
    # F at the four centers
    cell_indices = cell_index(p_i, xi_vx)
    Fi = @inbounds extract_field_corners(F, cell_indices...)
    xci = ntuple(Val(N)) do i
        xi_vx[i][cell_indices[i]]
    end
    ...
    #### Velocity from pressure nodes
    xi_center = ntuple(Val(N)) do i
        # need to add ghost nodes
        grid = xi_vx[N + (1-i)]
        LinRange(grid[1] - dxi[i] / 2, grid[end] - dxi[i] / 2, length(grid) + 2)
    end
    # F at the four centers
    cell_indices = cell_index(p_i, xi_center) 
    Fi = @inbounds extract_field_corners(Fc, cell_indices...)
    xci = ntuple(Val(N)) do i
        Base.@_inline_meta
        xi_center[i][cell_indices[i]]
    end
    # normalize particle coordinates
    t = normalize_coordinates(p_i, xci, dxi)
    # Interpolate field F onto particle
