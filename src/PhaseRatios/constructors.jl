...
function PhaseRatios(
    ...
    vertex = cell_array(B, zero(T), (nphases,), ni .+ 1)
    Vx = cell_array(B, zero(T), (nphases,), (nx + 1, ny))
    Vy = cell_array(B, zero(T), (nphases,), (nx, ny + 1))
    dummy = cell_array(B, zero(T), (nphases,), (1, 1)) # because it cant be a Union{T, Nothing} type on the GPU....
...
function PhaseRatios(
    ...
    center = cell_array(B, zero(T), (nphases,), ni)
    vertex = cell_array(B, zero(T), (nphases,), ni .+ 1)
    Vx = cell_array(B, zero(T), (nphases,), (nx + 1, ny, nz))
    Vy = cell_array(B, zero(T), (nphases,), (nx, ny + 1, nz))
    Vz = cell_array(B, zero(T), (nphases,), (nx, ny, nz + 1))
    yz = cell_array(B, zero(T), (nphases,), (nx, ny + 1, nz + 1))
    xz = cell_array(B, zero(T), (nphases,), (nx + 1, ny, nz + 1))
    xy = cell_array(B, zero(T), (nphases,), (nx + 1, ny + 1, nz))
