...
@parallel_indices (inode, jnode, knode) function grid2particle_classic!(Fp, F, xvi, di::NTuple{3, Any}, particle_coords)
    _grid2particle_classic!(
...
function grid2particle!(Fp::AbstractArray, xvi, F::AbstractArray, F0::AbstractArray, particle_coords; α = 0.0)
    di = grid_size(xvi)
    ni = length.(xvi)
...
@parallel_indices (inode, jnode) function grid2particle_full!(Fp, F, F0, xvi, di::NTuple{2, Any}, particle_coords, α)
    _grid2particle_full!(
        Fp, particle_coords, xvi, di, F, F0, (inode, jnode), α
    )
    return nothing
end

@parallel_indices (inode, jnode, knode) function grid2particle_full!(Fp, F, F0, xvi, di::NTuple{3, Any}, particle_coords, α)
    _grid2particle_full!(
        Fp, particle_coords, xvi, di, F, F0, (inode, jnode, knode), α
    )
    return nothing
end
...
@inline function _grid2particle_full!(Fp, p, xvi, di::NTuple{N, T}, F, F0, idx, α) where {N, T}
    # iterate over all the particles within the cells of index `idx` 
    for ip in cellaxes(Fp)
        # cache particle coordinates 
        pᵢ = ntuple(i -> (@cell p[i][ip, idx...]), Val(N))
