using JustPIC, CUDA, MAT, CellArrays, StaticArrays, ParallelStencil
...
@parallel_indices (ix,iy) function initialize_particles!(pX,pY,pC,pT,pA,rad2,lx,ly,dx,dy,np)
...
@parallel_indices (ix,iy) function copy_particles!(pX_old,pY_old,pC_old,pT_old,pA_old,pX,pY,pC,pT,pA)
    if ix <= size(pX,1) && iy <= size(pX,2)
...
@parallel_indices (ix,iy) function advect!(pX,pY,pX_old,pY_old,pC,pC_old,pT,pT_old,pA,pA_old,Vx,Vy,dt,dx,dy)
    if ix <= size(pX,1) && iy <= size(pX,2)
        nx,ny = size(pX)
...
function main()
    ...
    @time while it <= nt
        ...
        println(" # it = $it")
        @parallel copy_particles!(pX_old,pY_old,pC_old,pT_old,pA_old,pX,pY,pC,pT,pA)
        @parallel p2g!(C,T,pC,pT,pX,pY,pA)
        T_old  .= T
        @parallel advect!(pX,pY,pX_old,pY_old,pC,pC_old,pT,pT_old,pA,pA_old,Vx,Vy,dt,dx,dy)
