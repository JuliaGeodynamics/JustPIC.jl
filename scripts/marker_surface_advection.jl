...
yv = LinRange(0, Ly, n)
zv = LinRange(0, Lz, n)
function extend_centers(x)
    xc = [(x[i] + x[i + 1]) / 2 for i in 1:(length(x) - 1)]
...
Vx = TA(backend)([0.10 * sin(π * x) * cos(π * z) for x in grid_vxi[1][1], y in grid_vxi[1][2], z in grid_vxi[1][3]])
Vy = TA(backend)([0.0 for x in grid_vxi[2][1], y in grid_vxi[2][2], z in grid_vxi[2][3]])
Vz = TA(backend)([-0.10 * cos(π * x) * sin(π * z) for x in grid_vxi[3][1], y in grid_vxi[3][2], z in grid_vxi[3][3]])
V = Vx, Vy, Vz
