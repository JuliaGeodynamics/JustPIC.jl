...
function periodic_topography(n; x0 = 0.9)
    xv = yv = LinRange(0, 1, n)
    wrapped_distance(x, x0) = min(abs(x - x0), 1 - abs(x - x0))
    return [0.5 + 0.05 * exp(-50 * (wrapped_distance(x, x0)^2 + (y - 0.5)^2)) for x in xv, y in yv]
end

function marker_surface_example(topo; periodic_1 = false, suffix = "", elevation_range = nothing, velocity_resolution = 33, dt = 0.02, nt = 10)
    ...
    V = if periodic_1
        # Uniform x-translation: the bump exits one side and re-enters the other
        (
            TA(backend)(fill(0.2, length.(grid_vxi[1]))),
            TA(backend)(fill(0.0, length.(grid_vxi[2]))),
            TA(backend)(fill(0.0, length.(grid_vxi[3]))),
        )
    else
        # Divergence-free convection cell that uplifts the terrain
        (
            TA(backend)([0.1 * sin(π * x) * cos(π * z) for x in grid_vxi[1][1], y in grid_vxi[1][2], z in grid_vxi[1][3]]),
            TA(backend)(fill(0.0, length.(grid_vxi[2]))),
            TA(backend)([-0.1 * cos(π * x) * sin(π * z) for x in grid_vxi[3][1], y in grid_vxi[3][2], z in grid_vxi[3][3]]),
        )
    end
    ...
    for _ in 1:nt
        advect_marker_surface!(surf, V, grid_vxi, dt; max_slope_angle = 0)
    end
...
marker_surface_example(periodic_topography(33); periodic_1 = true, suffix = "_periodic", dt = 0.05, nt = 25)
